...
f = open('ManyMat_NegGradient.mat','w+')
...
for n in range(1,Nx+1): #VAMPIRE is 1-indexed
    ...
    if (n < Start):
        ...
        T = Tmax-(n-Start)* (Tmax-Tmin)/Nprofile 
        f.write(f'material['+str(n)+']:damping-constant='+str(alpha)+'\n')
        f.write(f'material['+str(n)+']:temperature='+str(T)+' \n')
    ...
    #f.write(f'material['+str(n)+']:initial-spin-direction=random \n')
...
plt.savefig('TempAndDampingProfile_negGrad.pdf')
plt.show()
