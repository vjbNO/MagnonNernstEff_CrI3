...
def plotDiffBetweenTempGrads(d1,d2,whichgrad):
    ...
    cbar.ax.set_title(r'$\Delta \mu^x$')
    ...
    if (whichgrad=='xgrad'):
        ...
        ax2.set_ylabel(r'$\langle \Delta\mu^x \rangle_x$',fontsize=25)
    else:
        longdir = np.arange(len(sig[0]))
        s,e=int(len(sig[0])/4),int(3*len(sig[0])/4)
        mean1 = np.mean(sig[s:e,:],axis=0)
        ax2.set_xlabel('x',fontsize=20)
        ax2.set_ylabel(r'$\langle \Delta\mu^x \rangle_y$',fontsize=25)
...
if __name__ == '__main__':
    ...
    fig1.savefig('Plots/Kitaev_IP_PosMinusNegXgrad_MuX.pdf')
    fig2.savefig('Plots/Kitaev_IP_PosMinusNegXgrad_MuXav.pdf')
    
    
    fig3,fig4=plotDiffBetweenTempGrads(PosYgrad,NegYgrad,'ygrad')
    fig3.savefig('Plots/Kitaev_IP_PosMinusNegYgrad_MuX.pdf')
    fig4.savefig('Plots/Kitaev_IP_PosMinusNegYgrad_MuXav.pdf')
